...
export class BenchmarkFormatter {
  ...
  private formatSummary(current: BenchmarkResults, base?: BenchmarkResults): string {
    ...
    if (base) {
      ...
      const contextChange = current.summary.averageContextReduction - base.summary.averageContextReduction;
      const operationChange = current.summary.averageOperationReduction - base.summary.averageOperationReduction;
      ...
      const contextTarget = 50; // 50% reduction target
      const operationTarget = 60; // 60% reduction target
      const speedupTarget = 2; // 2x speedup target
      
      output += `| **Context Reduction** | ${this.formatPercentage(current.summary.averageContextReduction)} | ≥${contextTarget}% | ${this.getStatusIcon(current.summary.averageContextReduction >= contextTarget)} |\n`;
      output += `| **Operation Reduction** | ${this.formatPercentage(current.summary.averageOperationReduction)} | ≥${operationTarget}% | ${this.getStatusIcon(current.summary.averageOperationReduction >= operationTarget)} |\n`;
      output += `| **Average Speedup** | ${current.summary.averageSpeedup.toFixed(1)}x | ≥${speedupTarget}x | ${this.getStatusIcon(current.summary.averageSpeedup >= speedupTarget)} |\n`;
    }
  ...
  private formatDetailedResults(current: BenchmarkResults, base?: BenchmarkResults): string {
    ...
    for (const [category, results] of Object.entries(categorizedResults)) {
      ...
      for (const result of results) {
        ...
        const contextIndicator = baseResult ? this.getComparisonIndicator(
          result.improvement.contextReduction,
          baseResult.improvement.contextReduction
        ) : '';
        
        const operationIndicator = baseResult ? this.getComparisonIndicator(
          result.improvement.operationReduction,
          baseResult.improvement.operationReduction
        ) : '';
        
        const speedupIndicator = baseResult ? this.getComparisonIndicator(
          result.improvement.speedup,
          baseResult.improvement.speedup
        ) : '';
  ...
  private formatStatusSummary(current: BenchmarkResults): string {
    ...
    if (failed.length > 0) {
      ...
      for (const scenario of failed) {
        ...
        output += `(target: ${scenario.expectedReduction?.context || 50}%), `;
        output += `Operations ${scenario.improvement.operationReduction.toFixed(0)}% `;
        output += `(target: ${scenario.expectedReduction?.operations || 60}%)\n`;
      }
      output += '\n</details>\n\n';
    }
  ...
  private categorizeResults(results: BenchmarkResult[]): Record<string, BenchmarkResult[]> {
    ...
    for (const result of results) {
      ...
      if (result.name.toLowerCase().includes('navigate') || 
          result.name.toLowerCase().includes('definition')) {
        category = 'Navigation';
      } else if (result.name.toLowerCase().includes('edit') || 
        ...
        category = 'Search';
      } else if (result.name.toLowerCase().includes('diagnostic') ||
